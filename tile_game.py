...
def draw_blocks():
    ...
    for pos, colr in blocks.copy().items():
        ...
        if pause == False:
            blocks[(pos[0][0], pos[0][1]+speed), (pos[1][0], pos[1][1]+speed),
                   (pos[2][0], pos[2][1]+speed), (pos[3][0], pos[3][1]+speed)] = colr
            del blocks[pos]
        if pos[0][1] > 600 and pause == False:
            del blocks[(pos[0][0], pos[0][1]+1), (pos[1][0], pos[1][1]+1),
                       (pos[2][0], pos[2][1]+1), (pos[3][0], pos[3][1]+1)]
            if colr != 5:
                life -= 1
    ...
    if mouse[1] < 560 and mouse[1] > 440:
        for pos, i in blocks.copy().items():
            for j, cord in enumerate(pos):
                if (mouse[0]-mouse[0] % 60 == cord[0]) and mouse[1] in range(cord[1], cord[1]+40) and (i == j):
                    score += 1
                    del blocks[pos]
                    blocks[pos] = 5
                    break
